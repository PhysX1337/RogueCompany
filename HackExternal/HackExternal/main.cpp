		...
		if (CFG.b_Visual)
		{
			if (entity_distance < CFG.max_distance)
			{
				...
				if (CFG.b_EspHealth)
				{
					...
					HealthBar(TopBox.x - (CornerWidth / 2) - 8, TopBox.y, width, BottomBox.y - TopBox.y, Percentage, true);
				}
...
void Render()
{
	...
	if (CFG.b_MenuShow)
	{
		...
		ImGui::Begin("Jenrix RogueCompany || jenrix#8201", 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
		...
		if (CFG.tab_index == 0)
		{
			...
			if (CFG.b_Visual)
			{
				...
				ImGui::SliderFloat("Max Distance", &CFG.max_distance, 1.0f, 500.0f);
